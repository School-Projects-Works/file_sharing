...
class FileServices{
  static final CollectionReference fileCollection = FirebaseFirestore.instance.collection('files');
  ...
  static Stream<List<FileModel>> getFiles() {
    return fileCollection.snapshots().map((snapshot) => snapshot.docs.map((doc) => FileModel.fromMap(doc.data() as Map<String, dynamic>)).toList());
  }

  static Future<String> uploadFile(Uint8List data, String id) async {
    try {
      var ref = storage.ref().child('files/$id'); 
      await ref.putData(data, SettableMetadata(contentType: 'pdf'));
